...
void server::operator()(boost::system::error_code ec, std::size_t length)
{
	...
	if (!ec) {
		...
		reenter(this)
		{
			...
			// saves the current
			// line number and exits the coroutine's "reenter"
			// block. We use the
			// server coroutine as the completion handler for the
			// async_accept
			// operation. When the asynchronous operation completes,
			// the io_service
			// invokes the function call operator, we "reenter" the
			// coroutine, and
			// then control resumes at the following line.
			yield acceptor_->async_accept(*socket_, *this);

			// We "fork" by cloning a new server coroutine to handle
			// the connection.
			// After forking we have a parent coroutine and a child
			// coroutine. Both
			// parent and child continue execution at the following
			// line. They can
			// be distinguished using the functions
			// coroutine::is_parent() and
			// coroutine::is_child().
			fork server (*this)();

			// The parent continues looping to accept the next
			// incoming connection.
			// The child exits the loop and processes the
			// connection.
			}
		while (is_parent());

		// Create the objects needed to receive a request on the
		// connection.
		...
		request_.reset(new request);
			...
			// An indeterminate result means we need more data, so
			// keep looping.
			}
		...
		yield boost::asio::async_write(*socket_, reply_->to_buffers(),
						   *this);
