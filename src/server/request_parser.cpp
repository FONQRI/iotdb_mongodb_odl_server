...
boost::tribool request_parser::consume(request &req, char c)
{
	...
	yield return boost::indeterminate;
		...
		while ((is_char(c) && !is_ctl(c) && !is_tspecial(c) && c != '\r') ||
		   (c == ' ' || c == '\t')) {
			if (c == ' ' || c == '\t') {
			// Leading whitespace. Must be continuation of previous
			// header's value.
			...
			req.headers.push_back(header());
				...
				// Header name.
				while (is_char(c) && !is_ctl(c) &&
				   !is_tspecial(c) && c != ':') {
				req.headers.back().name.push_back(c);
				yield return boost::indeterminate;
				}

			// Colon and space separates the header name from the
			// header value.
			if (c != ':')
				return false;
			yield return boost::indeterminate;
	...
	if (c != '\n')
		...
		for (std::size_t i = 0; i < req.headers.size(); ++i) {
			if (headers_equal(req.headers[i].name,
